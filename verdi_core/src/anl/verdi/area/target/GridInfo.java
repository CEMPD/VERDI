/** NOTE: This class uses ucar for projection class.
 *  BUT anl.verdi.area.target.ProjectionInfo uses GeoTools and OpenGis for CoordinateReferencingSystem (crs)
 *  and the Well Known Text (WKT) projection information including the .vt templates.
 */
...
public class GridInfo{
	...
	private static void computeMapDomain(final Projector projector,
		...
		if (Numerics.aboutEqual(mapDomain[LATITUDE][MINIMUM],
			...
			mapDomain[LATITUDE][MAXIMUM] = Numerics.sign(mapDomain[LATITUDE][MAXIMUM]) * 90.0;
			...
			mapDomain[LATITUDE][MAXIMUM] = Math.max(mapDomain[LATITUDE][MAXIMUM], longitudeLatitude[LATITUDE]);
