...
/**
 * A Swing GUI component for displaying a {@link org.jfree.chart.JFreeChart} object.
 * <p/>
...
public class ChartPanel extends JPanel
	...
	private void renderOffscreen(boolean scale, Rectangle2D chartArea, Rectangle2D available) {
		do {
			if (chartBuffer.validate(getGraphicsConfiguration()) == VolatileImage.IMAGE_INCOMPATIBLE) {
				createBackBuffer(available);
			}
			Rectangle2D bufferArea = new Rectangle2D.Double(0, 0, this.chartBufferWidth, this.chartBufferHeight);
			Graphics2D bufferG2 = this.chartBuffer.createGraphics();
			if (scale) {
		...
		} while (chartBuffer.contentsLost());
	}

	private void createBackBuffer(Rectangle2D available) {
		if (chartBuffer != null) {
			chartBuffer.flush();
			chartBuffer = null;
		}
	...
	public void paintComponent(Graphics g) {
		...
		do {
			int ret = chartBuffer.validate(getGraphicsConfiguration());
			if (ret == VolatileImage.IMAGE_RESTORED || refreshBuffer) {
				renderOffscreen(scale, chartArea, available);
				refreshBuffer = false;
			} else if (ret == VolatileImage.IMAGE_INCOMPATIBLE) {
				createBackBuffer(available);
				renderOffscreen(scale, chartArea, available);
			}
