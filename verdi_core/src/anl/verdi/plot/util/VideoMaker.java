...
//	private static final MessageCenter msg = MessageCenter.getMessageCenter(VideoMaker.class);
	...
	public VideoMaker(int frameRate, File file, String movieType) {
		this.videoType = movieType;
	...
	public boolean init(RGBFormat format) {
		...
		// set the output content descriptor to the video type
		p.setContentDescriptor(new FileTypeDescriptor(videoType));
	...
	/**
	 * Adds an image to a video as a frame with a default sleep value of 40
	 *
	...
	/**
	 * Adds an image to a video as a frame. This method pause the main
	...
	public void addImageAsFrame(BufferedImage image, int sleepCount) throws IOException {
		...
		if (!init) {
			...
			if (!result) {
				System.err.println("Failed to setup video capture");
			}
			init = true;
		}
		source.addBuffer(b);
