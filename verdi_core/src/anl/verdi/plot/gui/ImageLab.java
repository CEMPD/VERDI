...
public class ImageLab {
    ...
    public static void which(String s)
    {
        ...
        s = s.replace('.', '/');
        s = s + ".class";
        URL url = (new ImageLab()).getClass().getResource(s);
        if(url != null)
            System.out.println("\nClass '" + s + "' found in \n'" + url.getFile() + "'");
        else
    ...
    public static void main(String[] args) throws Exception {
    	...
    	try {
    		...
    		//which("org.ejml.data.DMatrixD1");
            Class<?> myClass = InterpolationNearest.class; 

            // Get the constructor with specific parameter types (if any)
            Constructor<?> constructor = myClass.getConstructor();

            // Create an instance of the class using the constructor
            Object myObject = constructor.newInstance(); 

    	n = new InterpolationNearest();
    	} catch (Throwable t) {
    		Throwable pr = null;
    		while (t instanceof NoClassDefFoundError) {
    			pr = t;
    			t = t.getCause();
    		}
    		if (t == null) {
    			System.out.println(pr.getCause());
    			pr.printStackTrace();
    		}
    		t.printStackTrace();
    	}*/
...
*/        //File imageFile = (File) wizard.getConnectionParameters().get("image");
   ...
   private void displayLayers(File rasterFile, File shpFile) throws Exception {
       ...
       System.out.println("Raster format " + format.getClass().getName() + " " + format);
       Hints hints = new Hints();
       if (format instanceof GeoTiffFormat) {
           hints = new Hints(Hints.FORCE_LONGITUDE_FIRST_AXIS_ORDER, Boolean.TRUE);
       }
       reader = format.getReader(rasterFile, hints);
       System.out.println("Reader " + reader.getClass().getName() + " " + reader);
