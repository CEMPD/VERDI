...
public class VerdiStyle implements Callable<Boolean> {
	...
	public GridCoverage2D imageCoverage = null;
	...
	public void projectImage(Projection proj, CoordinateReferenceSystem targetCRS) {
		sourceProjection = proj;
		projectingCRS = targetCRS;
	    ...
	    //GridCoverage2D image = new GeoTiffReader(f).read(new GeneralParameterValue[]{(GeneralParameterValue) policy, gridsize, useJaiRead});
	    try {
	    	if (imageCoverage == null) {
				imageCoverage = imageReader.read(new GeneralParameterValue[]{(GeneralParameterValue) policy, gridsize, useJaiRead});
			    imageCoverage = (GridCoverage2D) Operations.DEFAULT.resample(imageCoverage, vCRS);
	    	} else
	    		System.err.println("VerdiStyle using cached projected image");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public List<Layer> getLayers()			// get Layer based on FeatureSource and Style
	{
		if(vLayer == null)
		{
			if (isShapefile) {
				...
				if (imageCoverage == null)
					vLayer = new GridReaderLayer(imageReader, vStyle);
				else
					vLayer = new GridCoverageLayer(imageCoverage, vStyle);
			}
		}
		if (layerList == null) {
