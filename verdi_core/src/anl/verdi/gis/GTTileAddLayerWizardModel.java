...
// From FastTileAddLayerWizardModel
/**
 * @author User #2
 * @version $Revision: 1.4 $ $Date: 2009/05/12 19:26:12 $
 */
public class GTTileAddLayerWizardModel extends MultiPathModel {

	static final Logger Logger = LogManager.getLogger(GTTileAddLayerWizardModel.class.getName());
	private File mapFile;
	private VerdiBoundaries layer;
	private FeatureLayer controlLayer;

	public GTTileAddLayerWizardModel(Path path) {
		super(path);
		Logger.debug("done with GTTileAddLayerWizardModel constructor for path = " + path);
	}
