...
//From FastTileLayerPanel
/**
 * @author IE, UNC at Chapel Hill
 */
public class GTTileLayerPanel extends JPanel {
	private static final long serialVersionUID = -1216281930499819824L;
	static final Logger Logger = LogManager.getLogger(GTTileLayerPanel.class.getName());
	...
	// identity is based on the layer, as we only want a single LayerMove in the set per layer.
...
//		Object[] selected = layerList.getSelectedValues();	// JList getSelectedValues is now GetSelectedValuesList and returns List<>
	...
	private void addBtnActionPerformed(ActionEvent e) {	// removed for VERDI 1.5.0, reinstated for VERDI 1.6.0
		...
		GTTileAddLayerWizard wizard = new GTTileAddLayerWizard();
	...
	private void editLayerButtonPerformed(ActionEvent e) {
		Logger.debug("in GTTileLayerPlot.editLayerButtonPerformed");
		...
		Logger.debug("ready to instantiate GTTileAddLayerWizard");
		GTTileAddLayerWizard wizard = new GTTileAddLayerWizard(mapFile, controlLayer, selected, false);
		...
		if (layer != null && !selected.equals(layer)) {
			...
			Logger.debug("done dealing with layers, ready to return from GTTileLayerPlot.editLayerButtonPerformed");
			return;
		}
		Logger.debug("returning without dealing with layers, from GTTileLayerPlot.editLayerButtonPerformed");
	}
...
//				}));
	...
	public static void main(String[] args) {
		...
		frame.add(new GTTileLayerPanel(), BorderLayout.CENTER);
