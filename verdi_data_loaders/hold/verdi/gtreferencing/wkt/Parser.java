...
package verdi.gtreferencing.wkt;
...
//    public Parser(Symbols default1, DatumFactory datumFactory2,
//			CSFactory csFactory2, CRSFactory aCRSFactory,
...
//	}
     ...
     */
    public static boolean isValid(final Object parsed, final String keyword) {
        if (parsed == null) {
    ...
    private Map<String,Object> parseAuthority(final Element parent, final String name)
            throws ParseException
    {
    	System.out.println("into parseAuthority");
        ...
        System.out.println("got element = pullOptionalElement('AUTHORITY')");
        Map<String,Object> properties;
        System.out.println("instantiated Map properties");
        if (element == null) {
            ...
            if (code == null) {
            	System.out.println("code == null so looking for integer");
            	int codeNumber = element.pullInteger("code");
            	code = String.valueOf(codeNumber);
            	System.out.println("now code = " + code);
            }
            element.close();
            System.out.println("closed element, ready to call Citations.fromName");
            ...
            System.out.println("put NamedIdentifier values into properties");
        }
        if (isRoot) {
        	System.out.println("isRoot is TRUE, calling alterProperties");
            properties = alterProperties(properties);
            System.out.println("back from alterProperties");
        }
        System.out.println("ready to return properties from parseAuthority");
        return properties;
    }
    ...
    private <T extends Quantity> Unit<T> parseUnit(final Element parent, final Unit<T> unit)
            throws ParseException
    {
    	System.out.println("into parseUnit");
        final Element element = parent.pullElement("UNIT");
        System.out.println("back from pullElement, now call pullString");
        final String     name = element.pullString("name");
        System.out.println("name = " + name);
        final double   factor = element.pullDouble("factor");
        System.out.println("back from pullDouble, factor = " + factor);
        final Map<String,?> properties = parseAuthority(element, name);
        System.out.println("back from parseAuthority");
        element.close();
        System.out.println("done with element.close, ready to compute and return unit for factor = "+ factor);
    ...
    private PrimeMeridian parsePrimem(final Element parent, final Unit<Angle> angularUnit)
            throws ParseException
    {
    	System.out.println("into parsePrimem");
        final Element   element = parent.pullElement("PRIMEM");
        final String       name = element.pullString("name");
        final double  longitude = element.pullDouble("longitude");
        final Map<String,?> properties = parseAuthority(element, name);
        System.out.println("done with parseAuthority, ready to call datumFactory.createPrimeMeridian & return");	// OK TO HERE
        element.close();
        try {
        	PrimeMeridian aPrimeMeridian = datumFactory.createPrimeMeridian(properties, longitude, angularUnit);
        	System.out.println("calculated aPrimeMeridian: " + aPrimeMeridian);		// FAILS TO GET HERE
        	return aPrimeMeridian;
//            return datumFactory.createPrimeMeridian(properties, longitude, angularUnit);
    ...
    private Ellipsoid parseSpheroid(final Element parent) throws ParseException {
    	System.out.println("in parseSpheroid");
        Element          element = parent.pullElement("SPHEROID");
        System.out.println("just did parent.pullElement('SPHEROID')");
        String              name = element.pullString("name");
        System.out.println("just pulled name: " + name);
        double     semiMajorAxis = element.pullDouble("semiMajorAxis");
        System.out.println("pulled semiMajorAxis: " + semiMajorAxis);
        double inverseFlattening = element.pullDouble("inverseFlattening");
        System.out.println("pulled inverseFlattening: " + inverseFlattening);
        Map<String,?> properties = parseAuthority(element, name);
        System.out.println("pulled parseAuthority");
        element.close();
        System.out.println("closed element");
        if (inverseFlattening == 0) {
            // Inverse flattening null is an OGC convention for a sphere.
        	System.out.println("into inverseFlattening == 0");
            inverseFlattening = Double.POSITIVE_INFINITY;
        }
        try {
        	System.out.println("ready to call & return datumFactory.createFlattenedSphere");
            ...
            throws ParseException
    {
        ...
        System.out.println("in Parser.parseDatum, element = " + element.toString()); 		// OK TO THIS POINT
        String              name       = element.pullString("name");
        System.out.println("in Parser.parseDatum, name = " + name); 		// OK TO THIS POINT
        Ellipsoid           ellipsoid  = parseSpheroid(element);
        System.out.println("ellipsoid: just computed"); // + ellipsoid.toString());	// NoClassDefFoundError: Could not initialize class org.geotools.metadata.iso.citation.Citations
        BursaWolfParameters toWGS84    = parseToWGS84(element); // Optional; may be null.
        System.out.println("toWGS84 just computed");
        System.out.println("toWGS84: " + toWGS84.toString());				// OK TO THIS POINT
        Map<String,Object>  properties = parseAuthority(element, name);
        System.out.println("properties: " + properties.toString());			// DOES NOT GET HERE
        if (ALLOW_ORACLE_SYNTAX && (toWGS84 == null) && (element.peek() instanceof Number)) 
        {
        	System.out.println("into if ALLOW_ORACLE_SYNTAX etc.");
        ...
        System.out.println("done with 'if' in parseDatum");
        element.close();
        System.out.println("closed element");
        if (toWGS84 != null) {
        	System.out.println("true for toWGS84 != null");
            if (!(properties instanceof HashMap)) {
            	System.out.println("ready to get properties from new HashMap");
                properties = new HashMap<String,Object>(properties);
            }
            properties.put(DefaultGeodeticDatum.BURSA_WOLF_KEY, toWGS84);
            System.out.println("did properties.put - done with for toWGS84 != null");
        }
        try {
        	System.out.println("ready to try datumFactory.createGeodeticDatum");	// OK TO HERE
        	System.out.println("properties: " + properties.toString());
        	System.out.println("ellipsoid: " + ellipsoid.toString());
        	System.out.println("meridian: " + meridian.toString());
     ...
     */
    public EngineeringCRS parseLocalCS(final Element parent) throws ParseException {
     ...
     */
    public GeocentricCRS parseGeoCCS(final Element parent) throws ParseException {
        final Element          element = parent.pullElement("GEOCCS");
        final String              name = element.pullString("name");
        final Map<String,?> properties = parseAuthority(element, name);
        final PrimeMeridian   meridian = parsePrimem   (element, NonSI.DEGREE_ANGLE);
        System.out.println("ready to call parseDatum from parseGeoCCS");
        ...
        try {
            ...
            element.close();
            System.out.println("ready to call createGeocentricCRS & return from parseGeoCCS");
     ...
     */
    public VerticalCRS parseVertCS(final Element parent) throws ParseException {
     ...
     */
    public GeographicCRS parseGeoGCS(final Element parent) throws ParseException {
    	System.out.println("into parseGeoGCS");
        Element            element = parent.pullElement("GEOGCS");
        String                name = element.pullString("name");
        System.out.println("got name = " + name);
        Map<String,?>   properties = parseAuthority(element, name);
        System.out.println("back from parseAuthority, ready to call parseUnit for SI.RADIAN");	// OK TO HERE
        Unit<Angle>    angularUnit = parseUnit     (element, SI.RADIAN);
        System.out.println("back from parseUnit, ready to call parsePrimem");
        PrimeMeridian     meridian = parsePrimem   (element, angularUnit);
        System.out.println("done with units; meridian = " + meridian);		// DOES NOT GET HERE
        GeodeticDatum        datum = parseDatum    (element, meridian);
        System.out.println("done with parseDatum, datum = " + datum );
        CoordinateSystemAxis axis0 = parseAxis     (element, angularUnit, false);
        System.out.println("done with parseAxis");
        ...
        try {
            ...
            element.close();
            System.out.println("closed element, ready to instantiate ellipsoidalCS");
            ...
            System.out.println("ready to call createGeographicCRS & return from parseGeoGCS");
     ...
     * @throws ParseException if the "GEOGCS" element can't be parsed.
     */
    public ProjectedCRS parseProjCS(final Element parent) throws ParseException {
    	System.out.flush();
    	System.out.println("in Parser.parseProjCS");
    	System.out.println("argument named parent: " + parent.toString());
        Element                element = parent.pullElement("PROJCS");
        String                    name = element.pullString("name");
        Map<String,?>       properties = parseAuthority(element, name);
        System.out.println("in Parser.parseProjCS: back from parseAuthority");		// OK TO HERE
        GeographicCRS           geoCRS = parseGeoGCS(element);
        System.out.println("in Parser.parseProjCS: back from parseGeoGCS");			// FAILS BEFORE HERE

        Ellipsoid            ellipsoid = geoCRS.getDatum().getEllipsoid();
        System.out.println("in Parser.parseProjCS, ellipsoid: " + ellipsoid.toString());
        Unit<Length>        linearUnit = parseUnit(element, SI.METER);
        System.out.println("in Parser.parseProjCS: back from parseUnit: " + linearUnit.toString());

        Unit<Angle>        angularUnit = geoCRS.getCoordinateSystem().getAxis(0).getUnit().asType(Angle.class);
        System.out.println("in Parser.parseProjCS: back from angularUnit: " + angularUnit.toString());

        ParameterValueGroup projection = parseProjection(element, ellipsoid, linearUnit, angularUnit);
        System.out.println("in Parser.parseProjCS: back from parseProjection: " + projection.toString());

        CoordinateSystemAxis     axis0 = parseAxis(element, linearUnit, false);
        System.out.println("in Parser.parseProjCS: back from parseAxis, axis0: " + axis0.toString());

        CoordinateSystemAxis     axis1;
        try {
            if (axis0 != null) {
                axis1 = parseAxis(element, linearUnit, true);
                System.out.println("axis0 was not null so back from parseAxis for axis1: " + axis1.toString());
            } else {
                // Those default values are part of WKT specification.
                axis0 = createAxis(null, "X", AxisDirection.EAST,  linearUnit);
                System.out.println("created axis0");
                axis1 = createAxis(null, "Y", AxisDirection.NORTH, linearUnit);
                System.out.println("created axis1");
            }
            element.close();
            System.out.println("closed element, ready to call DefiningConversion");
            final Conversion conversion = new DefiningConversion(name, projection);
            System.out.println("back from DefiningConversion; ready to call createProjectedCRS");
            return crsFactory.createProjectedCRS(properties, geoCRS, conversion,
                    csFactory.createCartesianCS(properties, axis0, axis1));
        } catch (FactoryException exception) {
        	System.out.println("caught an exception; ready to through element.parseFailed");
            throw element.parseFailed(exception, null);	// 2014: if element was closed above, how can it be used here and then not closed?
        }
    }
     ...
     */
    public CompoundCRS parseCompdCS(final Element parent) throws ParseException {
     ...
     * @throws ParseException if the "COMPD_CS" element can't be parsed.
     */
    public DerivedCRS parseFittedCS(final Element parent) throws ParseException {
