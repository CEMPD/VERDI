...
public  class Element {
     ...
     */
    public Element(final Element singleton) {
        offset  = 0;
        keyword = null;
        list    = new LinkedList<Object>();
        list.add(singleton);
        System.out.println("in constructor Element(Element) and set keyword=null");
    }
     ...
     */
    public Element(final AbstractParser parser, final String text, final ParsePosition position)
            throws ParseException
    {
         ...
         */
    	System.out.println("in Element constructor from parser, text, & ParsePosition");
        ...
        System.out.println("now lower: " + lower + " and upper: " + upper);
        if (upper <= lower) {
            position.setErrorIndex(lower);
            System.out.println("in error block for upper <= lower & going to throw unparsableString");
        ...
        list = new LinkedList<Object>();
        System.out.println("created new list as a LinkedList<Object>");
        /*
        ...
        do {
            if (position.getIndex() >= length) {
            	System.out.println("ready to throw missingCharacter");
            ...
            if (parseOptionalSeparator(text, position, parser.symbols.quote)) {
                ...
                if (upper < lower) {
                    position.setErrorIndex(++lower);
                    System.out.println("from if parseOptionalSeparator ready to through missingCharacter");
            ...
            if (!Character.isUnicodeIdentifierStart(text.charAt(lower))) {
                ...
                if (number == null) {
                	...
                	System.out.println("number == null so ready to through unparsableString");
                    throw unparsableString(text, position);
                }
                list.add(number);
                continue;
            ...
            System.out.println("adding as a child element in string");
        } while (parseOptionalSeparator(text, position, parser.symbols.separator));
        System.out.println("ready to call parseSeparator");
        parseSeparator(text, position, parser.symbols.closingBrackets[bracketIndex]);
        System.out.println("back from parseSeparator");
    }
