// $Id:GeoTiff.java 63 2006-07-12 21:50:51Z edavis $
/*
...
public class GeoTiff {
  ...
  private ArrayList geokeys = new ArrayList();
  ...
  private void writeGeoKeys() {
    ...
    for (int i=0; i<geokeys.size(); i++) {
      GeoKey geokey = (GeoKey) geokeys.get(i);
      if (geokey.isDouble)
  ...
  private void writeIFD( FileChannel channel, int start) throws IOException {
    ...
    for (int i=0; i<tags.size(); i++) {
      IFDEntry elem = (IFDEntry) tags.get(i);
  ...
  private int writeIntValue(ByteBuffer buffer, IFDEntry ifd, int v) {
    switch ( ifd.type.code) {
        case 1 : buffer.put((byte) v);
          return 1;
        case 3 : buffer.putShort((short)v);
          return 2;
        case 4 : buffer.putInt(v);
          return 4;
        case 5 : buffer.putInt(v);
          return 4;
      }
    return 0;
  }
...
*/
  ...
  private int readIntValue(ByteBuffer buffer, IFDEntry ifd) {
    switch ( ifd.type.code) {
        case 1 : return (int) buffer.get();
        case 2 : return (int) buffer.get();
        case 3 : return readUShortValue( buffer);
        case 4 : return buffer.getInt();
        case 5 : return buffer.getInt();
      }
    return 0;
  }
